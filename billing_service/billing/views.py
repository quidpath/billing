...
from .models import Plan, Invoice, Payment, PaymentVerification
...
def list_plans(request):
    ...
    try:
        ...
        return JsonResponse({'success': True, 'data': {'plans': plans_data}}, status=200)
    ...
    try:
        ...
        return JsonResponse({
            ...
            'data': {
                ...
                'corporate_id': str(subscription.corporate_id),  # Return for verification
...
def get_subscription_status(request):
    ...
    try:
        ...
        if not subscription:
            return JsonResponse({'success': True, 'data': {'subscription': None}}, status=200)
...
def validate_promotion(request):
    ...
    try:
        ...
        if not promotion_code:
            return JsonResponse({'success': False, 'message': 'Promotion code required'}, status=400)
        ...
        for inv in invoices:
            if str(inv.corporate_id) == str(corporate_id):
                invoices_data.append({
                    'id': str(inv.id),
                    'corporate_id': str(inv.corporate_id),  # Include for verification
...
def initiate_payment(request):
    ...
    try:
        ...
        logger.info(f"Payment initiation request: invoice_id={invoice_id}, invoice_number={invoice_number}, payment_method={payment_method}, customer_email={customer_email}, customer_phone={customer_phone}, corporate_id={corporate_id}")
        
        if not invoice_id and not invoice_number:
            logger.warning(f"Payment initiation failed: Missing invoice_id and invoice_number")
            return JsonResponse({
                'success': False,
                'message': 'Invoice ID or invoice number required'
            }, status=400)
        
        if not payment_method or not customer_email:
            logger.warning(f"Payment initiation failed: payment_method={payment_method}, customer_email={customer_email}")
            return JsonResponse({
                'success': False,
        ...
        if payment_method == 'mpesa' and not customer_phone:
            logger.warning(f"Payment initiation failed: M-Pesa payment requires phone number")
            return JsonResponse({
                'success': False,
        ...
        is_valid, error_msg = validate_corporate_id(corporate_id)
        if not is_valid:
            logger.warning(f"Payment initiation failed: Invalid corporate_id={corporate_id}, error={error_msg}")
            return JsonResponse({
                'success': False,
                'message': error_msg or 'Corporate ID is required for payment security'
            }, status=400)
        ...
        if not invoice:
            logger.warning(f"Payment initiation failed: Invoice not found - invoice_id={invoice_id}, invoice_number={invoice_number}")
        ...
        if invoice.status == 'paid':
            return JsonResponse({'success': False, 'message': 'Invoice already paid'}, status=400)
        ...
        if payment_method == 'mpesa':
            ...
            if test_mode:
                ...
                callback_url = os.environ.get('MPESA_CALLBACK_URL', 'https://webhook.site/unique-id-here')
            else:
                # Production requires a real publicly accessible URL
                callback_url = os.environ.get('MPESA_CALLBACK_URL', f'{request.scheme}://{request.get_host()}/api/billing/payments/webhook/mpesa/')
            ...
            if not all([provider_config['consumer_key'], provider_config['consumer_secret'], provider_config['passkey']]):
                return JsonResponse({
                    'success': False,
            ...
            provider_config = {
                ...
                'callback_url': os.environ.get('PAYSTACK_CALLBACK_URL', f'{request.scheme}://{request.get_host()}/api/billing/payments/webhook/'),
            ...
            provider_config = {
                ...
                'test_mode': os.environ.get('PESAWAY_TEST_MODE', 'true').lower() == 'true',
        ...
        import traceback
        logger = logging.getLogger(__name__)
        logger.info(f"Initiating payment: method={payment_method}, invoice={invoice.invoice_number}, amount={invoice.total_amount}, phone={customer_phone}")
        ...
        try:
            ...
            # Log result
            logger.info(f"Payment initiation result: success={result.get('success')}, message={result.get('message')}")
            ...
            logger.info(f"Payment initiation response: success={result.get('success')}, payment_id={result.get('payment_id')}, provider_reference={result.get('provider_reference')}")
            
            return JsonResponse(result, status=200 if result['success'] else 400)
        except Exception as payment_error:
            # Log the full error with traceback
            error_traceback = traceback.format_exc()
            logger.error(f"Error initiating payment: {str(payment_error)}\n{error_traceback}")
...
def payment_webhook(request):
    ...
    try:
        ...
        logger.info(f"Webhook received: path={request.path}, payload keys={list(payload.keys())}")
        ...
        logger.info(f"Webhook received: path={request.path}, provider={provider}, payload keys={list(payload.keys())}")
        ...
        logger.info(f"Webhook processing result: success={result.get('success')}, payment_id={result.get('payment_id')}, message={result.get('message')}")
        ...
        if result.get('success') and result.get('payment_id'):
            try:
                ...
                result['data']['invoice_corporate_id'] = str(payment.invoice.corporate_id) if payment.invoice else None
                ...
                logger.info(f"Webhook: Payment {result['payment_id']} updated successfully. Payment Status: {payment.status}, Invoice ID: {invoice_id}, Invoice Status: {invoice_status}")
            except Payment.DoesNotExist:
                logger.warning(f"Webhook: Payment {result.get('payment_id')} not found after webhook processing")
        ...
        if subscription and subscription.status == 'active':
            ...
            return JsonResponse({
                ...
                'unpaid_invoices': [{
                    ...
                    'due_date': inv.due_date.isoformat(),
                } for inv in unpaid_invoices[:5]],  # Return up to 5 most recent
...
def check_payment_status(request):
    ...
    try:
        ...
        logger.info(f"check_payment_status called: payment_id={payment_id}, corporate_id={corporate_id}")
        ...
        try:
            ...
            logger.info(f"Payment found: id={payment.id}, status={payment.status}, method={payment.payment_method}, provider_ref={payment.provider_reference}")
        except Payment.DoesNotExist:
        ...
        if str(payment.corporate_id) != str(corporate_id):
            logger.warning(f"Unauthorized payment access: payment.corporate_id={payment.corporate_id}, request.corporate_id={corporate_id}")
            return JsonResponse({
                'success': False,
        ...
        if payment.status in ['failed', 'cancelled']:
            ...
            return JsonResponse({
                ...
                'payment': {
                    ...
                    'invoice_id': str(payment.invoice.id) if payment.invoice else None,
                    'invoice_status': payment.invoice.status if payment.invoice else None,
                    'failure_reason': payment.metadata.get('failure_reason', '') if hasattr(payment, 'metadata') else None,
                }
            }, status=200)
        ...
        if payment.payment_method == 'mpesa' and payment.provider_reference:
            ...
            logger.info(f"Querying M-Pesa payment status for payment_id={payment_id}, CheckoutRequestID={payment.provider_reference}")
            result = adapter.verify_payment(payment.provider_reference)
            logger.info(f"M-Pesa query result for payment {payment_id}: status={result.get('status')}, message={result.get('message')}")
            ...
            if result.get('status') == 'success':
                ...
                logger.info(f"Payment {payment_id} confirmed as successful via M-Pesa query")
                ...
                logger.info(f"Payment {payment_id} confirmed as failed via M-Pesa query: {failure_message}")
                ...
                logger.info(f"Payment {payment_id} still pending. M-Pesa result: {result}")
                return JsonResponse({
                    'success': True,
                    'payment': {
                        'id': str(payment.id),
        ...
        for pmt in payments:
            if str(pmt.corporate_id) == str(corporate_id):
                payments_data.append({
                    ...
                    'invoice_number': pmt.invoice.invoice_number if pmt.invoice else None,
                    'paid_at': pmt.paid_at.isoformat() if pmt.paid_at else None,
                    'created_at': pmt.created_at.isoformat(),
                    'failure_reason': pmt.failure_reason if hasattr(pmt, 'failure_reason') else None,
                })
...
def admin_corporate_summary(request, corporate_id):
    ...
    try:
        ...
        invoices_data = [{
            ...
            'due_date': inv.due_date.isoformat(),
            'paid_at': inv.paid_at.isoformat() if inv.paid_at else None,
        } for inv in invoices[:10]]  # Return last 10 invoices
...
def mpesa_webhook(request):
    ...
    try:
        ...
        if not stk_callback:
            ...
            return JsonResponse({'success': False, 'message': 'Missing stkCallback'}, status=400)
        ...
        try:
            ...
            logger.warning(f"M-Pesa webhook: Payment not found for CheckoutRequestID: {checkout_request_id}")
            return JsonResponse({
                'success': False,
                'message': f'Payment not found for CheckoutRequestID: {checkout_request_id}'
            }, status=404)
        
        logger.info(f"Processing M-Pesa webhook for payment {payment.id}, ResultCode: {result_code}")
        ...
        if result_code == 0:
            ...
            logger.info(f"Payment {payment.id} marked as successful via webhook. Receipt: {receipt_number}")
            ...
            logger.warning(f"Payment {payment.id} marked as failed via webhook. ResultCode: {result_code}, Reason: {failure_reason}")
