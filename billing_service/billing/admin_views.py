...
from .models import Invoice, Payment, Plan, Promotion, Subscription, Trial
from .services import SubscriptionService, TrialService
    ...
    try:
        ...
        stats = {
            ...
            "invoices": {
                ...
                "total_amount": float(
                    Invoice.objects.aggregate(Sum("total_amount"))["total_amount__sum"]
                    or 0
                ),
                "pending_amount": float(
                    Invoice.objects.filter(status__in=["pending", "overdue"]).aggregate(
                        Sum("total_amount")
                    )["total_amount__sum"]
                    or 0
                ),
        ...
        return JsonResponse({"success": True, "data": stats}, status=200)
    except Exception as e:
        return JsonResponse({"success": False, "message": str(e)}, status=500)
...
def get_corporate_billing_summary(request, corporate_id):
    ...
    try:
        ...
        # Get trial status
        trial = (
            Trial.objects.filter(corporate_id=corporate_id)
            .order_by("-created_at")
            .first()
        )
        trial_data = None
        ...
        subscription = (
            Subscription.objects.filter(corporate_id=corporate_id, status="active")
            .select_related("plan")
            .first()
        )
        ...
        # Get invoices
        invoices = Invoice.objects.filter(corporate_id=corporate_id).order_by(
            "-created_at"
        )[:10]
        invoices_data = [
            {
        ...
        # Get payments
        payments = Payment.objects.filter(corporate_id=corporate_id).order_by(
            "-created_at"
        )[:10]
        payments_data = [
            {
        ...
        total_invoiced = float(
            Invoice.objects.filter(corporate_id=corporate_id).aggregate(
                Sum("total_amount")
            )["total_amount__sum"]
            or 0
        )
        ...
        total_paid = float(
            Payment.objects.filter(
                corporate_id=corporate_id, status="completed"
