...
class VerificationService:
    ...
    def initiate_verification(
        ...
        try:
            ...
            existing = PaymentVerification.objects.filter(
                corporate_id=corporate_id, status="verified"
            ).first()
            ...
            verification = PaymentVerification.objects.create(
                ...
                + timedelta(minutes=VerificationService.VERIFICATION_EXPIRY_MINUTES),
            )
            ...
            if not plan:
                return {"success": False, "message": "No active plans available"}
            ...
            if result.get("success"):
                verification.payment_provider_reference = result.get(
                    "provider_reference"
                )
                ...
                verification.mark_as_failed(
                    result.get("message", "Failed to initiate verification payment")
                )
    ...
    def handle_verification_payment_success(
        provider_reference: str, receipt_number: str, metadata: Dict
    ) -> Dict:
        """
        ...
        try:
            ...
            if not payment or not payment.invoice:
                logger.warning(
                    f"Payment or invoice not found for reference {provider_reference}"
                )
            ...
            if not payment.invoice.metadata.get("is_verification"):
                ...
                return {"success": True, "message": "Not a verification payment"}
            ...
            verification = PaymentVerification.objects.filter(
                id=verification_id
            ).first()
            if not verification:
            ...
            if refund_result.get("success"):
                ...
                logger.error(
                    f"Failed to initiate refund for {verification_id}: {refund_result.get('message')}"
                )
    ...
    def initiate_refund(verification: PaymentVerification) -> Dict:
        ...
        try:
            ...
            if test_mode:
                ...
                logger.info(
                    f"Sandbox mode: Auto-marking verification {verification.id} as refunded"
                )
                ...
                logger.info(
                    f"Production mode: Marking verification {verification.id} for manual refund"
                )
                verification.mark_as_refunded(
                    refund_reference=f"RFND-{verification.id.hex[:8].upper()}",
                    ...
                    metadata={
                        ...
                        "amount": float(verification.verification_amount),
                    },
                )
    ...
    @staticmethod
    def create_trial_after_verification(
        verification_id: str, plan_tier: str = "starter"
    ) -> Dict:
        """
        ...
        try:
            ...
            return {
                ...
                "verification": {
                    ...
                    "refunded_at": (
                        verification.refunded_at.isoformat()
                        if verification.refunded_at
                        else None
                    ),
                    "trial_created": verification.trial_created,
                    "trial_id": (
                        str(verification.trial_id) if verification.trial_id else None
                    ),
    ...
    @staticmethod
    def handle_verification_payment_failed(
        provider_reference: str, reason: str
    ) -> Dict:
        """
        ...
        try:
            ...
            if verification:
                ...
                logger.info(
                    f"Verification {verification_id} marked as failed: {reason}"
                )
