...
class InvoiceService:
    ...
    def create_invoice_for_subscription(
        ...
        if subscription.additional_users > 0:
            InvoiceLineItem.objects.create(
                ...
                total_price=Decimal(str(subscription.additional_users))
                * subscription.additional_user_price,
                item_type="users",
            )
        ...
        try:
            ...
            logger.info(
                f"Invoice {invoice.invoice_number} created for corporate {invoice.corporate_id}"
            )
    ...
    def get_corporate_invoices(corporate_id: str, limit: int = 50) -> list:
        ...
        return list(
            Invoice.objects.filter(corporate_id=corporate_id).order_by("-created_at")[
                :limit
            ]
        )
