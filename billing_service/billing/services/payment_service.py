...
class PaymentService:
    ...
    def initiate_payment(
        ...
        if result.get('status') == 'success':
            payment.provider_reference = result.get('provider_reference') or result.get('merchant_request_id')
    ...
    def handle_payment_webhook(
        ...
        if not payment:
            ...
            if not payment:
                logger.error(f"Payment not found for reference {provider_reference} or CheckoutRequestID {checkout_request_id}")
                return {
                    'success': False,
                    'message': 'Payment not found',
                }
        
        logger.info(f"Webhook: Found payment {payment.id} for reference {provider_reference}, current status: {payment.status}")
        ...
        if webhook_data.get('status') == 'success':
            ...
            logger.info(f"Webhook: Payment {payment.id} marked as success. Invoice status: {payment.invoice.status if payment.invoice else 'N/A'}")
            ...
            if payment.invoice and payment.invoice.metadata.get('is_verification'):
                ...
                receipt_number = webhook_data.get('metadata', {}).get('mpesa_receipt', '')
            ...
            try:
                logger.info(f"Payment confirmed for invoice {payment.invoice.invoice_number}")
            ...
            if payment.invoice and payment.invoice.metadata.get('is_verification'):
                ...
                VerificationService.handle_verification_payment_failed(
                    provider_reference,
                    webhook_data.get('message', 'Payment failed')
                )
