...
class RemoteAuthBackend(ModelBackend):
    ...
    def authenticate(self, request, username=None, password=None, **kwargs):
        ...
        logger.info(
            f"Local authentication failed for '{username}', trying remote authentication"
        )
    ...
    def _authenticate_remote(self, username, password):
        ...
        main_backend_url = getattr(
            settings, "MAIN_BACKEND_URL", "http://django-backend-dev:8000"
        )
        ...
        try:
            ...
            response = requests.post(
                verify_url, json={"username": username, "password": password}, timeout=5
            )
            ...
            if response.status_code == 200:
                ...
                if data.get("success"):
                    ...
                    logger.warning(
                        f"Remote authentication failed for '{username}': {data.get('error')}"
                    )
                    return None
            else:
