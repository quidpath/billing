...
class Plan(BaseModel):
    ...
    quarterly_discount_percent = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal('5.00'))
    yearly_discount_percent = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal('20.00'))
    
    # User Limits
    included_users = models.IntegerField(default=3)  # Users included in base price
    additional_user_price = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('800.00'))
    max_users = models.IntegerField(null=True, blank=True)  # null = unlimited
    
    # Feature Limits (JSON)
    limits = models.JSONField(default=dict, blank=True)  # e.g., {"invoices": 50, "storage_gb": 5}
...
class PlanFeature(BaseModel):
    ...
    description = models.TextField(blank=True)
    feature_key = models.CharField(max_length=100, unique=True)  # e.g., "ai_analyses", "api_calls"
    is_active = models.BooleanField(default=True)
...
class PlanModule(BaseModel):
    ...
    price_starter = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'))
    price_professional = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'))
    price_business = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'))
    price_enterprise = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'))
    ...
    def get_price_for_tier(self, tier: str) -> Decimal:
        ...
        price_map = {
            ...
            'enterprise': Decimal('0.00') if self.included_in_enterprise else self.price_enterprise,
        }
...
class PlanModuleAssignment(BaseModel):
    ...
    """
    plan = models.ForeignKey(Plan, on_delete=models.CASCADE, related_name='module_assignments')
    module = models.ForeignKey(PlanModule, on_delete=models.CASCADE, related_name='plan_assignments')
