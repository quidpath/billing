...
class Promotion(BaseModel):
    ...
    )  # List of plan tiers: ['starter', 'professional']
    ...
    max_uses_per_customer = models.IntegerField(
        default=1
    )  # How many times one customer can use
    ...
    free_modules = models.JSONField(
        default=list, blank=True
    )  # List of module types to give free
    ...
    def can_be_used_by_customer(self, corporate_id: str) -> bool:
        ...
        usage_count = PromotionUsage.objects.filter(
            promotion=self, corporate_id=corporate_id
        ).count()
