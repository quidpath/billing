...
class Subscription(BaseModel):
    ...
    def save(self, *args, **kwargs):
        """Override save to trigger webhooks"""
        ...
        # Send webhooks after save
        from billing_service.billing.services.webhook_service import \
            webhook_service

        if is_new:
            webhook_service.send_subscription_created(self)
        elif old_status and old_status != self.status:
            if self.status == "active":
                webhook_service.send_subscription_activated(self)
            elif self.status == "cancelled":
                webhook_service.send_subscription_cancelled(self)
            elif self.status == "expired":
                webhook_service.send_subscription_expired(self)
...
class SubscriptionHistory(BaseModel):
    ...
    action = models.CharField(
        max_length=50
    )  # created, activated, cancelled, renewed, etc.
