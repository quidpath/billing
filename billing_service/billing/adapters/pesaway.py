...
class PesawayAdapter(PaymentAdapter):
    ...
    def __init__(self, config: Dict):
        ...
        self.base_url = config.get("base_url") or (
            "https://api.pesaway.com"
            if not self.test_mode
            else "https://sandbox.pesaway.com"
        )
        ...
        try:
            ...
            if payment_method in ["mpesa", "airtel_money"]:
                if not customer_phone:
                    return {
                        "status": "failed",
                        "message": f"{payment_method.upper()} requires customer phone number",
                    }
    ...
    def handle_webhook(self, payload: Dict, headers: Dict) -> Dict:
        try:
            ...
            transaction_id = payment_data.get("transaction_id") or payment_data.get(
                "reference"
            )
