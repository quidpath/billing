...
class MpesaDarajaAdapter(PaymentAdapter):
    ...
    def _get_access_token(self) -> str:
        ...
        try:
            ...
            if not self.consumer_key or not self.consumer_secret:
                raise Exception("M-Pesa Consumer Key and Consumer Secret must be configured")
            ...
            if response.status_code == 400:
                ...
                raise Exception(f"M-Pesa authentication failed: {error_detail}. Please verify your Consumer Key and Consumer Secret are correct.")
            
            if response.status_code == 401:
                logger.error("M-Pesa authentication failed (401): Unauthorized")
                raise Exception("M-Pesa authentication failed: Unauthorized. Please verify your Consumer Key and Consumer Secret are correct.")
            ...
            if not self._access_token:
                ...
                raise Exception("M-Pesa access token not received. Please check your credentials.")
            ...
            if "M-Pesa authentication failed" in str(e) or "Failed to connect" in str(e):
                raise
    ...
    def initiate_payment(
        ...
        try:
            ...
            account_ref = metadata.get('invoice_number', 'QUIDPATH') if metadata else 'QUIDPATH'
            transaction_desc = metadata.get('description', 'Quidpath Subscription Payment') if metadata else 'Quidpath Subscription Payment'
            ...
            logger.info(f"Initiating M-Pesa STK Push for phone {phone}, amount {amount}")
            ...
            if response.status_code == 200 and response_data.get('ResponseCode') == '0':
                ...
                customer_message = response_data.get('CustomerMessage', 'STK push sent to your phone')
                
                logger.info(f"STK Push successful - CheckoutRequestID: {checkout_request_id}, MerchantRequestID: {merchant_request_id}")
                ...
                error_code = response_data.get('errorCode') or response_data.get('ResponseCode')
                error_message = response_data.get('errorMessage') or response_data.get('ResponseDescription', 'Failed to initiate payment')
                
                logger.error(f"M-Pesa STK Push failed - Code: {error_code}, Message: {error_message}")
    ...
    def verify_payment(self, provider_reference: str) -> Dict:
        ...
        try:
            ...
            if not response.text or not response.text.strip():
                logger.error(f"M-Pesa query returned empty response. Status: {response.status_code}")
                return {
                    'status': 'failed',
            ...
            try:
                ...
                logger.error(f"M-Pesa query returned invalid JSON. Status: {response.status_code}, Response: {response.text[:200]}")
                
                # For 403/401 errors, treat as pending to allow retry (might be temporary auth issue)
                if response.status_code in [403, 401]:
                    logger.warning(f"M-Pesa authentication error ({response.status_code}). Keeping payment as pending for retry.")
                    return {
                        'status': 'pending',
            ...
            if response.status_code == 200:
                ...
                if result_code == '0':
                    ...
                    status_message = result_desc or response_data.get('ResultDesc', 'Payment pending')
                    logger.info(f"M-Pesa payment status: ResultCode={result_code}, ResultDesc={status_message}")
                    return {
                        'status': 'pending',
                ...
                logger.warning(f"M-Pesa rate limit hit (429). Payment still pending, will retry.")
                return {
                    'status': 'pending',
                ...
                logger.error(f"M-Pesa query returned non-200 status: {response.status_code}, Response: {response.text[:200]}")
                return {
                    'status': 'failed',
